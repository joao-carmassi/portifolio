---
title: "Button"
description: "A versatile button component with multiple variants, sizes, visual effects, icons, and loading states. Built on Radix UI and Tailwind CSS."
---
...
<Install registryUrl='/components/button.json' />

The direct URL serves the `radix-vega` variant. The Button is style-aware — its sizes, radius, and type scale differ per shadcn style. To let the CLI pick the variant that matches your project's theme automatically, register the `@prisma` namespace once (see [Styles & Themes](/components/styles/)) and install by name:

<Install registryUrl='@prisma/button' />
