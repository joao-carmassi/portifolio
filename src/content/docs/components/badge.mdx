---
title: "Badge"
description: "A compact label component for status indicators, tags, and categories. Supports variants, sizes, and visual effects."
---
...
<Install registryUrl='/components/badge.json' />

The direct URL serves the `radix-vega` variant. The Badge is style-aware — its sizes, radius, and type scale differ per shadcn style. To let the CLI pick the variant that matches your project's theme automatically, register the `@prisma` namespace once (see [Styles & Themes](/components/styles/)) and install by name:

<Install registryUrl='@prisma/badge' />
