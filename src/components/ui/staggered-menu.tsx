...
export interface StaggeredMenuProps {
  ...
  accentColor?: string;
  isFixed?: boolean;
  /** drive the menu from a parent; leave undefined to keep it self-managed */
  open?: boolean;
  /** the component ships its own logo + toggle bar; turn it off to supply your own */
  showHeader?: boolean;
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // a parent that passes `open` drives the same path the button would
  React.useEffect(() => {
    if (controlledOpen === undefined) return;
    if (controlledOpen === openRef.current) return;
    toggleMenu();
  }, [controlledOpen, toggleMenu]);
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  ...
                  >
                    <a
                      className='sm-panel-item relative text-black font-semibold text-5xl cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]'
                      href={it.link}
                ...
                <li
                  ...
                  aria-hidden='true'
                >
                  <span className='sm-panel-item relative text-black font-semibold text-5xl cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]'>
                    <span className='sm-panel-itemLabel inline-block origin-[50%_100%] will-change-transform'>
                      No items
                    </span>
                  </span>
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; background: white; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; box-shadow: 0 0 40px rgba(0, 0, 0, 0.25); }
...
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 2.3em; font-size: 18px; font-weight: 400; color: var(--sm-accent, #ff0000); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
