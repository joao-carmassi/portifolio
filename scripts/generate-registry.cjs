...
const uiDir = path.join(__dirname, '..', 'src', 'prisma', 'ui');
...
// The base style: its sources live in src/prisma/ui and are also used as
...
// from src/prisma/ui/styles/{style}/, everything else (and the base style)
// from the canonical src/prisma/ui/. A missing per-style file falls back to
...
// the docs keep prisma under src/prisma so it never collides with the
// portfolio's own components; installs get the usual shadcn paths back
function toShadcnPaths(content) {
  return content
